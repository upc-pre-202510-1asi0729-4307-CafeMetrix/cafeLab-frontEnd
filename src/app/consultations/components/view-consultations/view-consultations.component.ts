...
export class ViewConsultationsComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.coffeeService.getCoffees().subscribe({
      next: (coffees: Coffee[]) => {
        // Filtra cafés por usuario
        this.coffees = coffees.filter(c => String(c.userId) === String(currentUserId));
        this.defectService.getDefects().subscribe({
          next: (defects: Defect[]) => {
            // Filtra defectos por usuario
            this.defects = defects.filter(d => String(d.userId) === String(currentUserId));
